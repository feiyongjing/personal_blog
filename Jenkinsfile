...
def normalCIBuild(String version) {
    ...
    sh("docker build . -t 47.99.104.121:5000/xdml-springboot:${version}")

    sh("docker push 47.99.104.121:5000/xdml-springboot:${version}")
...
def deployVersion(String version) {
    sh "ssh -p 24 root@47.99.104.121 'docker rm -f xdml && docker run --name xdml -d -p 8081:8080 47.99.104.121:5000/xdml-springboot:${version}'"
}
...
def rollback() {
    def dockerRegistryHost = "http://47.99.104.121:5000";
